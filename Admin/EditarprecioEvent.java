...
public class EditarprecioEvent implements Initializable {
    @FXML
    ...
    private void setprecio(){
        try {
            if(producto.getItems().size()==0){}
            else {
                String pr = producto.getSelectionModel().getSelectedItem().toString();
                vprecio.setText(metodos.get_prop_producto(tipo.getSelectionModel().getSelectedItem().toString(), pr.substring(0, pr.indexOf(":")), "precio"));
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public void cambio(){
        String pr=producto.getSelectionModel().getSelectedItem().toString();
        try {
            metodos.editarprecio(tipo.getSelectionModel().getSelectedItem().toString(),pr.substring(0,pr.indexOf(":")),nprecio.getText());
